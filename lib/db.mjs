...
export async function ensureIndexes() {
  ...
  // Migration: pre-scan.mjs deployments created `linkedin_id_1` as a plain
  // unique index. With scan.mjs jobs that have linkedin_id=null, we need a
  // partial-filter unique so the nulls don't collide. Mongo refuses to
  // replace an index in place — drop first if shape doesn't match, then
  // recreate below. Safe no-op on fresh clusters (dropIndex errors if the
  // index is absent, which we catch).
  const jobsIdx = await db.collection('jobs').indexes().catch(() => []);
  const linkedinIdx = jobsIdx.find(i => i.name === 'linkedin_id_1');
  if (linkedinIdx && !linkedinIdx.partialFilterExpression) {
    console.error('[db] migrating jobs.linkedin_id_1 → partial unique (dropping + recreating)');
    await db.collection('jobs').dropIndex('linkedin_id_1').catch(() => {});
  }

  await db.collection('jobs').createIndexes([
    { key: { linkedin_id: 1 },                     name: 'linkedin_id_1', unique: true, partialFilterExpression: { linkedin_id: { $type: 'string' } } },
...
/**
 * Upsert a job keyed by `url` (for sources without a linkedin_id — e.g.
 * greenhouse/ashby/lever). Mirrors upsertJob's first-sighting vs.
 * re-sighting semantics.
 */
export async function upsertJobByUrl(doc) {
  const db = await getDb();
  const now = new Date();
  const { url, first_scan_run_id, ...fields } = doc;
  return await withRetry(() => db.collection('jobs').updateOne(
    { url },
    {
      $set: {
      ...
      $setOnInsert: {
        stage: 'raw',
        linkedin_id: fields.linkedin_id || null,
        first_seen_at: now,
        first_scan_run_id: first_scan_run_id || null,
        stage_history: [{ stage: 'raw', at: now, source: fields.source_type ? `scan-${fields.source_type}` : null }],
        cv_artifact_ids: [],
        application_id: null,
        report_id: null,
      },
...
/**
 * For URL-keyed dedup: given an array of URLs we just fetched, return a
 * Set of those already present in the jobs collection.
 */
export async function findJobsBySeenUrls(urls) {
  const db = await getDb();
  if (!urls || urls.length === 0) return new Set();
  const docs = await withRetry(() => db.collection('jobs')
    .find({ url: { $in: urls } }, { projection: { url: 1, _id: 0 } })
    .toArray());
